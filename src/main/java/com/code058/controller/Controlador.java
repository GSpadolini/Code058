...
public class Controlador {
    ...
    private void anadirPedido() {
        ...
        // DESCUENTO como BigDecimal
        BigDecimal descuentoBD = BigDecimal.valueOf(1 - clienteParaPedido.descuentoEnvio());

        // GASTO DE ENVÍO = BigDecimal (proteger null)
        BigDecimal gastoArticulo = (articuloParaPedido.getGastoEnvio() != null)
                ? articuloParaPedido.getGastoEnvio()
                : BigDecimal.ZERO;

        BigDecimal gastoEnvioBD = gastoArticulo.multiply(descuentoBD).setScale(2, RoundingMode.HALF_UP);

        // TIEMPO de preparación igual que antes
        ...
        nuevoPedido = new Pedido(clienteParaPedido, articuloParaPedido, numeroPedidoTemporal,
                cantidad, fechaPedido, gastoEnvioBD, tiempoPreparacion);
    ...
    private void eliminarPedido() {
        ...
        for (Pedido p : listaPedidos) {
            String emailCli = (p.getCliente() != null && p.getCliente().getEmail() != null) ? p.getCliente().getEmail() : "[sin cliente]";
            String codArt = (p.getArticulo() != null && p.getArticulo().getCodigo() != null) ? p.getArticulo().getCodigo() : "[sin articulo]";
